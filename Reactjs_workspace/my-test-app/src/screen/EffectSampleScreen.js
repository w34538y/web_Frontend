...
// 마운트(mount) : 새로운 컴포넌트를 DOM 상에 생성하는 것
// 업데이트 (update) : state, props의 변경으로 인해 컴포넌트의 갱신이 발생하는 것
// 언마운트 (unmount) : 컴포넌트가 DOM 상에서 제거되는 것

// 사이드이펙트(side effect) : 특정 함수가 호출되어 실핼될 때, 함수 내부가 아닌 외부에 영향을 미치는 것
/* 
    useEffect()
    컴포넌트가 마운트 된 후 ***
    컴포넌트가 업데이트 된 후
    ...
    특정 state가 변화할 때 마다 useEffect를 실행시키고 싶다면 [조건 배열] 자리에 그 state 값을 넣는다 
